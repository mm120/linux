...
#define XHCI_FWFILENAME_720201_202ES20  "renesas/K2017090.mem"
...
#define DEBUG_FORMAT_MODULE_NAME    "[XHCI FW Downloader] "
...
static XHCI_FWDOWNLOADER XHCI_CheckControlWithTimeOut (
    ...
    while(1) {
        ...
        now = ktime_get();
        sub_time = ktime_sub(now, start);
        if(ktime_compare(sub_time, ktime_set(1, 0)) > 0 ) {
...
typedef enum {
    ...
    unsigned int fw_dwordsize   = nFWSize / (sizeof(unsigned int));

    DebugPrintFunctionName();

    if((nFWSize % (sizeof(unsigned int))) != 0) {
        fw_dwordsize++;
    }
...
XHCI_FWDOWNLOADER XHCI_FWDownLoadCheck(
	...
	xhci->firmware_pointer = NULL;
        ...
        for(loopcounter = 0; (loopcounter < 10)&&(xhci->firmware_pointer == NULL); loopcounter++) {
            if(request_firmware(&xhci->firmware_pointer, firmwarename, &pDev->bus->dev) == 0) {
                XHCI_DebugPrint("Firmware load OK");
                result = XHCI_FWDownLoad(pDev, xhci->firmware_pointer->data, xhci->firmware_pointer->size);
